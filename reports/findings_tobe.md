...
| HIGH | 61 | `EU-RET-001`, `EU-SPC-001`, `EU-TRP-001`, `KR-RET-001`, `KR-SPC-001`, `KR-TRF-001`, `KR-TRP-001` |
...
특별범주 · KR · 기본 심각도 **HIGH** · 대상 2건

근거: 개인정보 보호법 제23조 제1항

| 대상 | 발동 사유 |
|---|---|
| `CH-01` 주문 입력창 | `input_type` eq → "free_text" / `pii_prefilter` is_null → null / `is_transferred` is_true → true |
| `CH-02` 고객센터 문의 | `input_type` eq → "free_text" / `pii_prefilter` is_null → null / `is_stored` is_true → true / `is_transferred` is_true → true |
